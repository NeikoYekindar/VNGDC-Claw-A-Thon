...
import { api, type Server, type VulnerabilityAssetDetail } from "@/lib/api";
import { osTypeLabel } from "@/lib/utils";
...
export default function VulnerabilityServerDetailPage({ params }: { params: { id: string } }) {
  ...
  const refresh = useCallback(async () => {
    setError(null);
    try {
      const [srv, assetDetail] = await Promise.all([
        api.server(id),
        api.vulnerabilityAsset(id),
      ]);
      setServer(srv);
  ...
  const runVulnerabilityRefresh = async () => {
    ...
    setRefreshing(true);
    try {
      await api.refreshVulnerabilities(true, false);
      await refresh();
    } catch (e) {
      setError(String(e));
    } finally {
      ...
      <div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          <div className="flex flex-wrap gap-2">
            <a
              href={api.vulnerabilityAssetExportUrl(id)}
